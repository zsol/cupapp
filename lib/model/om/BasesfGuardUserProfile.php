...
abstract class BasesfGuardUserProfile extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the last_commented field.
	 * @var        string
	 */
	protected $last_commented;

	/**
	 * The value for the last_uploaded field.
	 * @var        string
	 */
	...
	/**
	 * Get the [optionally formatted] temporal [last_commented] column value.
	 * 
	 *
	 * @param      string $format The date/time format string (either date()-style or strftime()-style).
	 *							If format is NULL, then the raw DateTime object will be returned.
	 ...
	 * @throws     PropelException - if unable to parse/validate the date/time value.
	 */
	public function getLastCommented($format = 'Y-m-d H:i:s')
	{
		if ($this->last_commented === null) {
			return null;
		}


		if ($this->last_commented === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->last_commented);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->last_commented, true), $x);
			}
		}
	...
	/**
	 * Get the [optionally formatted] temporal [last_uploaded] column value.
	 * 
	 *
	 * @param      string $format The date/time format string (either date()-style or strftime()-style).
	 *							If format is NULL, then the raw DateTime object will be returned.
	 ...
	 * @throws     PropelException - if unable to parse/validate the date/time value.
	 */
	public function getLastUploaded($format = 'Y-m-d H:i:s')
	{
		if ($this->last_uploaded === null) {
			return null;
		}


		if ($this->last_uploaded === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->last_uploaded);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->last_uploaded, true), $x);
			}
		}
	...
	public function setModifiedAt($v)
	{
		...
		if ( $this->modified_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = sfGuardUserProfilePeer::MODIFIED_AT;
			}
	...
	/**
	 * Sets the value of [last_commented] column to a normalized version of the date/time value specified.
	 * 
	 * @param      mixed $v string, integer (timestamp), or DateTime value.  Empty string will
	 *						be treated as NULL for temporal objects.
	 * @return     sfGuardUserProfile The current object (for fluent API support)
	 */
	public function setLastCommented($v)
	{
		// we treat '' as NULL for temporal objects because DateTime('') == DateTime('now')
		// -- which is unexpected, to say the least.
		if ($v === null || $v === '') {
		...
		if ( $this->last_commented !== null || $dt !== null ) {
			// (nested ifs are a little easier to read in this case)

			$currNorm = ($this->last_commented !== null && $tmpDt = new DateTime($this->last_commented)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d H:i:s') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->last_commented = ($dt ? $dt->format('Y-m-d H:i:s') : null);
				$this->modifiedColumns[] = sfGuardUserProfilePeer::LAST_COMMENTED;
			}
		} // if either are not null
	...
	/**
	 * Sets the value of [last_uploaded] column to a normalized version of the date/time value specified.
	 * 
	 * @param      mixed $v string, integer (timestamp), or DateTime value.  Empty string will
	 *						be treated as NULL for temporal objects.
	 * @return     sfGuardUserProfile The current object (for fluent API support)
	 */
	public function setLastUploaded($v)
	{
		// we treat '' as NULL for temporal objects because DateTime('') == DateTime('now')
		// -- which is unexpected, to say the least.
		if ($v === null || $v === '') {
		...
		if ( $this->last_uploaded !== null || $dt !== null ) {
			// (nested ifs are a little easier to read in this case)

			$currNorm = ($this->last_uploaded !== null && $tmpDt = new DateTime($this->last_uploaded)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d H:i:s') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->last_uploaded = ($dt ? $dt->format('Y-m-d H:i:s') : null);
				$this->modifiedColumns[] = sfGuardUserProfilePeer::LAST_UPLOADED;
			}
		} // if either are not null
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->last_commented = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
			$this->last_uploaded = ($row[$startcol + 6] !== null) ? (string) $row[$startcol + 6] : null;
			$this->resetModified();
			...
			return $startcol + 7; // 7 = sfGuardUserProfilePeer::NUM_COLUMNS - sfGuardUserProfilePeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			...
			case 4:
				...
				break;
			case 5:
				return $this->getLastCommented();
				break;
			case 6:
				return $this->getLastUploaded();
				break;
			default:
				return null;
				break;
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[5] => $this->getLastCommented(),
			$keys[6] => $this->getLastUploaded(),
		);
		return $result;
	}
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			...
			case 4:
				...
				break;
			case 5:
				$this->setLastCommented($value);
				break;
			case 6:
				$this->setLastUploaded($value);
				break;
		} // switch()
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setAvatar($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setModifiedAt($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setLastCommented($arr[$keys[5]]);
		if (array_key_exists($keys[6], $arr)) $this->setLastUploaded($arr[$keys[6]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(sfGuardUserProfilePeer::MODIFIED_AT)) $criteria->add(sfGuardUserProfilePeer::MODIFIED_AT, $this->modified_at);
		if ($this->isColumnModified(sfGuardUserProfilePeer::LAST_COMMENTED)) $criteria->add(sfGuardUserProfilePeer::LAST_COMMENTED, $this->last_commented);
		if ($this->isColumnModified(sfGuardUserProfilePeer::LAST_UPLOADED)) $criteria->add(sfGuardUserProfilePeer::LAST_UPLOADED, $this->last_uploaded);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setLastCommented($this->last_commented);

		$copyObj->setLastUploaded($this->last_uploaded);
