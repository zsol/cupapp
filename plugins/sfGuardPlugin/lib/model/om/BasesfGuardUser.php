...
abstract class BasesfGuardUser extends BaseObject  implements Persistent {
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related Replays from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collReplays === null) {
			if ($this->isNew()) {
			   $this->collReplays = array();
				...
				$criteria->add(ReplayPeer::USER_ID, $this->id);
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related ReplayComments from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collReplayComments === null) {
			if ($this->isNew()) {
			   $this->collReplayComments = array();
				...
				$criteria->add(ReplayCommentPeer::USER_ID, $this->id);
	 ...
	 * actually need in sfGuardUser.
	 */
	public function getReplayCommentsJoinReplay($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collReplayComments === null) {
			if ($this->isNew()) {
				...
				$this->collReplayComments = ReplayCommentPeer::doSelectJoinReplay($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
